...
class deletecustomtable {
	...
	public function get() {
		...
		if ($table = $database->getCustomTable(request('table'))) {
			$tpl = new Template('deletecustomtable.html');
			$tpl->setVar('tablename', request('table'));
			$tpl->setVar('description', $table['description']);
			$content = $tpl->get();
			return array('title'=>'IPDB :: Delete custom table',
						 'content'=>$content);
		} else
			return array('title'=>'Error',
						 'content'=>'Requested custom table cannot be found');
	}
